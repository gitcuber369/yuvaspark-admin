...
export const useStudentResponseStore = create<StudentResponseStore>((set, get) => ({
  ...
  fetchResponses: async (filters = {}) => {
    ...
    try {
      ...
      let url = 'http://192.168.1.3:3000/api/student-responses/scored';
      
      // If filters are provided, use the appropriate endpoint
      if (filters.studentId) {
        url = `http://192.168.1.3:3000/api/student-responses/student/${filters.studentId}`;
      }
  ...
  fetchByStudent: async (studentId: string) => {
    set({ loading: true, error: null });
    try {
      const url = `http://192.168.1.3:3000/api/student-responses/student/${studentId}`;
      const response = await fetch(url);
  ...
  fetchResponseById: async (id: string) => {
    set({ loading: true, error: null });
    try {
      const url = `http://192.168.1.3:3000/api/student-responses/${id}`;
      const response = await fetch(url);
  ...
  deleteResponse: async (id: string) => {
    set({ loading: true, error: null });
    try {
      const response = await fetch(`http://192.168.1.3:3000/api/student-responses/${id}`, {
        method: 'DELETE',
      });
  ...
  exportResponses: async (filters = {}) => {
    ...
    try {
      ...
      const url = `http://192.168.1.3:3000/api/student-responses/export?${queryParams.toString()}`;
      const response = await fetch(url);
