...
export default function CohortPage() {
  ...
  const availableTeachers = teachers.filter(
    teacher => !cohorts.some(
      cohort => cohort.teachers.some(t => t.id === teacher.id)
    )
  );
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                {availableTeachers.length === 0 ? (
                  <p className="text-sm text-gray-500">No available teachers to assign</p>
                ) : (
                  <div className="max-h-40 overflow-y-auto border rounded-md p-2">
                    {availableTeachers.map(teacher => (
                      <div key={teacher.id} className="flex items-center gap-2 p-1 hover:bg-gray-50">
                        <input
                          type="checkbox"
                        ...
                        <label htmlFor={`teacher-${teacher.id}`} className="text-sm">
                          {teacher.name} {teacher.anganwadi && `(${teacher.anganwadi.name})`}
                        </label>
                      </div>
                    ))}
                  </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {cohort.teachers.length > 0 ? (
                          cohort.teachers.map(teacher => (
                            <Badge key={teacher.id} variant="outline" className="bg-gray-100">
                              {teacher.name}
                            </Badge>
                          ))
                        ) : (
